...
| Name | Version |
|------|---------|
| <a name="requirement_google"></a> [google](#requirement\_google) | >= 6.0, < 9.0 |

## Providers

| Name | Version |
|------|---------|
| <a name="provider_google"></a> [google](#provider\_google) | >= 6.0, < 9.0 |
...
|------|-------------|------|---------|:--------:|
| <a name="input_authorised_networks"></a> [authorised\_networks](#input\_authorised\_networks) | Map of authorized networks to allow access. | `map(string)` | `{}` | no |
...
| <a name="input_edition"></a> [edition](#input\_edition) | The edition of the Database instance possible values are ENTERPRISE/ ENTERPRISE\_PLUS | `string` | `"ENTERPRISE"` | no |
...
| <a name="input_primary_psc_config"></a> [primary\_psc\_config](#input\_primary\_psc\_config) | (Optional) PSC config for primary | <pre>object({<br/>    psc_enabled               = optional(bool)<br/>    allowed_consumer_projects = optional(set(string))<br/>  })</pre> | `null` | no |
...
| <a name="input_settings_database_authentication_enabled"></a> [settings\_database\_authentication\_enabled](#input\_settings\_database\_authentication\_enabled) | (Optional, Default: `true`) Whether to enable IAM database authentication. When enabled, existing BUILT\_IN password users continue to work alongside IAM users (CLOUD\_IAM\_USER, CLOUD\_IAM\_SERVICE\_ACCOUNT, CLOUD\_IAM\_GROUP). | `bool` | `true` | no |
...
| <a name="input_settings_database_read_replica_flags"></a> [settings\_database\_read\_replica\_flags](#input\_settings\_database\_read\_replica\_flags) | List of Cloud SQL flags applied to the read replica. If null (default), the read replica inherits settings\_database\_flags. Set this to give read replicas a different flag set from the primary - for example to omit flags that require a larger instance. See [more details](https://cloud.google.com/sql/docs/mysql/flags) | `map(any)` | `null` | no |
